...
if (isset($_POST['titreSignal'])&&isset($_POST['content_signal'])&& isset($_POST['id_comment2']) && $array_config[0][1]==='on')
{
      $signal = new Signal(
            "",
            htmlentities(addslashes($_POST['titreSignal'])),
            htmlentities(addslashes($_POST['content_signal'])),
            htmlentities(addslashes($_SESSION['data_user'][0])),
            htmlentities(addslashes($_POST['id_comment2'])),'1'
      );
      $signal = $signal->create_signal();

      $updateComment = new Comment("","","","","","","");
      $updateComment->update_comment_statut('2',htmlentities(addslashes($_POST['id_comment2'])));
}
else if(isset($_POST['titreSignal']) && isset($_POST['content_signal']) && isset($_POST['id_comment2']))
{
      $signal = new Signal(
            ...
            htmlentities(addslashes($_POST['content_signal'])),
            '2',
            htmlentities(addslashes($_POST['id_comment2'])),
            '1'
      );
      $signal = $signal->create_signal();
      $updateComment = new Comment("","","","","","","");
